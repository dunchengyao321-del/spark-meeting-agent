...
# ---- A. 绑定校验
r = validate_binding(IN_MEETING, TITLE_FMT.format(no="123456789"), "123456789")
check("A1 会中且会议号匹配 → 绑定", r.ok and r.state == "bound")

r = validate_binding(IN_MEETING, TITLE_FMT.format(no="123456789"), "111222333")
check("A2 会议号不匹配 → 拒绝并给出实际会议号",
      not r.ok and r.state == "rejected" and r.candidates == ["123456789"])

r = validate_binding(IN_MEETING, "项目周会 - 飞书会议", "123456789")
check("A3 会中但读不到会议号 → 未校验绑定", r.ok and r.state == "unverified")

r = validate_binding("扫码登录 手机号登录", "", "123456789")
check("A4 登录态失效 → 拒绝", not r.ok and "登录" in r.reason)

r = validate_binding("首页 发起新会议 加入会议", "", "123456789")
check("A5 不在会中 → 拒绝", not r.ok and "不在会议中" in r.reason)

r = validate_binding(IN_MEETING, TITLE_FMT.format(no="123456789"), "58812")
check("A6 非 9 位会议号 → 拒绝", not r.ok and "9 位" in r.reason)

body = f"会议号 588 123 808 已复制 {IN_MEETING}"
r = validate_binding(body, "", "123456789")
check("A7 分段显示的会议号不误判为候选", r.ok)

# ---- B. 会议号提取
check("B1 9 位会议号提取", MEETING_NO_RE.findall("会议 123456789 和 111222333") == ["123456789", "111222333"])
...
# ---- D. 称呼枚举（示例：张三/三哥/张张/alex/张老师，大小写不敏感）
names = ["张三", "三哥", "张张", "alex", "张老师"]
arbiter = TurnArbiter(names)
check("D1 中文名触发", arbiter.decide("会议", "三哥，你怎么看？").action == "speak")
check("D2 英文称呼触发", arbiter.decide("会议", "alex 这个排期没问题吧").action == "speak")
check("D3 大小写不敏感", arbiter.decide("会议", "Alex 你觉得呢").action == "speak")
check("D4 张老师触发", arbiter.decide("会议", "让张老师说一下").action == "speak")
