...
市面上多数"会议机器人"走的是 SDK 机器人或虚拟声卡路线，这个项目选了更难但更通用的一条：**直接操作飞书会议网页版**。Playwright 启动 Chrome，`add_init_script` 注入 shim 层，拦截 `getUserMedia` 返回合成麦克风轨（上行发声）、包装 `RTCPeerConnection` 捕获远端音频轨（下行收听），页面内 WebSocket 双向传输 48kHz 单声道 PCM。任何能用网页版打开的会，分身都能进：不用装虚拟声卡，也不用改会议那侧的任何配置。
